...
class BootstrapCode extends CrudCode
{
	public function generateActiveRow($modelClass, $column)
	{
		if ($column->type === 'boolean') {
			...
			return "\$form->textAreaGroup(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50, 'class'=>'span8'))";
		} else {
			...
			if ($column->type !== 'string' || $column->size === null) {
				if($column->dbType == 'date') {
					return "\$form->datepickerGroup(\$model,'{$column->name}',array('options'=>array(),'htmlOptions'=>array('class'=>'span5')),array('prepend'=>'<i class=\"icon-calendar\"></i>','append'=>'Click on Month/Year at top to select a different year or type in (mm/dd/yyyy).'))";
				} else {
				...
				if (strpos ( $column->dbType, 'enum(' ) !== false) {
					...
					$dropdown_options .= ")";
					return "\$form->dropDownListGroup(\$model,'{$column->name}',{$dropdown_options},array('class'=>'input-large'))";
				} else {
